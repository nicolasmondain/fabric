...
export default {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        img.onload = () => {
          ...
          const h = homothetic.gethomothetic(
            img.width,
            img.height,
            finalSize.width,
            finalSize.height
  ...
  onecolordodge(d: number, d2: number): number {
    return Math.abs(d2) === globals.GRADIATION_MAX
      ? d2
      : Math.min(
          globals.GRADIATION_MAX,
          (d * globals.GRADIATION_MAX) / (globals.GRADIATION_MAX - d2)
        );
  },

  onecolorburn(d: number, d2: number): number {
    return Math.abs(d2) === 0
      ? d2
      : Math.max(
          0,
          globals.GRADIATION_MAX - ((globals.GRADIATION_MAX - d) * globals.GRADIATION_MAX) / d2
        );
  },

  onecolorvividlight(d: number, d2: number): number {
    return Math.abs(d2) < 128
      ? this.onecolorburn(d, 2 * d2)
      : this.onecolordodge(d, 2 * (d2 - 128));
  },

  onecolorreflect(d: number, d2: number): number {
    return d2 === globals.GRADIATION_MAX
      ? d2
      : Math.min(globals.GRADIATION_MAX, (d * d) / (globals.GRADIATION_MAX - d2));
  },
  ...
  hsltorgb(h: number, s: number, l: number): { r: number; g: number; b: number } {
    ...
    return {
      r: r * globals.GRADIATION_MAX,
      g: g * globals.GRADIATION_MAX,
      b: b * globals.GRADIATION_MAX,
    };
  },
};
