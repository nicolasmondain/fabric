...
export class Filters {
  ...
  constructor(
    fabric: library,
    configFilters: filtersConfigModule = {},
    type: filtersType = 'webgl'
  ) {
    this.fabric = fabric;
    ...
    this.ios =
      this.type === 'js'
        ? true
        : ['iPad', 'iPhone', 'iPod'].includes(window.navigator.platform) ||
  ...
  getconf(
    ...
    prefix = '' as string,
    medias = [] as Array<{ index: number; type: string; src: string }>
  ): filtersConfig {
      ...
      for (let i = 0; i < conf.actions.length; i += 1) {
        if (medias?.length && conf.actions[i].parameters.media) {
          const media = medias?.find(
            (m) =>
              m.index === conf.actions[i].parameters.media?.index &&
              m.type === conf.actions[i].parameters.media?.type
          ...
          const isBase64 = (imageData: string | ImageData | undefined) =>
            typeof imageData === 'string' ? imageData.slice(0, 5) === 'data:' : false; // eslint-disable-line no-confusing-arrow

          conf.actions[i].parameters.imageData2 = isBase64(imageData2)
            ? imageData2
            : prefix + conf.actions[i].parameters.imageData2;
          conf.actions[i].parameters.imageData3 = isBase64(imageData3)
            ? imageData3
            : prefix + conf.actions[i].parameters.imageData3;
        }
      }
    } else if (actions && Array.isArray(actions) && actions.length) {
      conf = {
        name,
        actions: actions?.filter((a) => a.name && typeof this.extensions[a.name] === 'function'),
        test: false,
        active: false,
        type: 'custom',
        force: 'auto',
  ...
  action(image: fabricImage, filter: string, actions: filtersActions): Promise<fabricImage> {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        const accumulator = (action: filtersAction) =>
          new Promise<void>((next) => {
            // eslint-disable-line max-lines-per-function, max-statements
            ...
            if (option.parameters?.frame) {
              ...
              if (
                typeof option.parameters?.imageData2 === 'string' &&
                prototype.configuration.imageData2
              ) {
                promises.push(methods.getimagedata(option.parameters.imageData2, size));
              }

              if (
                typeof option.parameters?.imageData3 === 'string' &&
                prototype.configuration.imageData3
              ) {
            ...
            Promise.allSettled(promises).then((results) => {
              ...
              option.parameters.imageData2 =
                results[0]?.status === 'fulfilled' ? (results[0].value as ImageData) : '';
              option.parameters.imageData3 =
                results[1]?.status === 'fulfilled' ? (results[1].value as ImageData) : '';
              ...
              if (LENTICULAR_CASE.includes(option.name)) {
                    ...
                    methods
                      ...
                      .then((imageData2) => {
                        ...
                        tmp.onload = () => {
                          ...
                          image.filters.push(
                            new this.fabric.Image.filters[action.name](option.parameters)
                          );
                ...
                img2.src = methods.getimageurl(option.parameters.imageData2 as ImageData, {
                  width,
                  height,
                });
              } else {
        ...
        [...actions]
          .reduce(
            (previous, current, i) => previous.then(() => accumulator(actions[i])),
            Promise.resolve()
          )
          .then(() => resolve(image));
      } catch (error) {
  ...
  apply(image: fabricImage, configuration: filtersConfig): Promise<fabricImage> {
    // eslint-disable-line
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (configuration.name && configuration.actions.length) {
          ...
          this.action(image, configuration.name, actions)
            .then(() => {
              this.type = SAVE_TYPE;
