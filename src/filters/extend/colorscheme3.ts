...
import {filtersApplyTo2dOptions, filtersApplyToWebglOptions, filtersDataArray, homothetic, library} from '../../@types';

import filtersdata from '../data';
import globals from '../globals';
import methods from '../methods';

export const colorscheme3 = (fabric: library) => {

	fabric.Image.filters.colorscheme3 = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
		...
		imageData2   : null,
		scheme       : filtersdata.colorscheme3 as filtersDataArray,
		value1       : 0,
		mainParameter: 'useBy',
		process      : 'current',
		...
		applyTo2d(options: filtersApplyTo2dOptions){
			...
			for(let i = 0; i < n; i += 4){
				...
				if(methods.applytothecurrenti(i, this.homothetic.x, this.homothetic.y, this.homothetic.w, this.homothetic.h, options.sourceWidth)){
					...
					data[i]     = 0.01 * ((data[i] * value1) + (this.value1 * filtersdata.colorscheme3[data[i]][2]));
					data[i + 1] = 0.01 * ((data[i + 1] * value1) + (this.value1 * filtersdata.colorscheme3[data[i + 1]][1]));
					data[i + 2] = 0.01 * ((data[i + 2] * value1) + (this.value1 * filtersdata.colorscheme3[data[i + 2]][0]));
	...
	fabric.Image.filters.colorscheme3.fromObject = fabric.Image.filters.BaseFilter.fromObject;
