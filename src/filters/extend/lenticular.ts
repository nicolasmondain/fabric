...
export const lenticular = (fabric: library) => {
	...
	fabric.Image.filters.lenticular = fabric.util.createClass(fabric.Image.filters.BaseFilter, {
		...
		applyTo2d(options: filtersApplyTo2dOptions){
			...
			const rule  = BAND_RULES.find((r) => (r.w === options.sourceWidth && r.h === options.sourceHeight) || (r.w === options.sourceHeight && r.h === options.sourceWidth));
			...
			if(n === m){
				...
				for(let i = 0; i < n; i += 4){
					...
					if(x >= this.incrustation.x && x <= this.incrustation.x + this.incrustation.w && y >= this.incrustation.y && y <= this.incrustation.y + this.incrustation.h){
