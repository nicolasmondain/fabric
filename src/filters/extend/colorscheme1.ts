		...
		applyTo2d(options: filtersApplyTo2dOptions){
			...
			for(let i = 0; i < n; i += 4){
				...
				if(methods.applytothecurrenti(i, this.homothetic.x, this.homothetic.y, this.homothetic.w, this.homothetic.h, options.sourceWidth)){
					...
					data[i + 1] = 0.01 * ((data[i + 1] * value1) + (this.value1 * filtersdata.colorscheme1[data[i + 1]][1]));
					data[i + 2] = 0.01 * ((data[i + 2] * value1) + (this.value1 * filtersdata.colorscheme1[data[i + 2]][0]));
